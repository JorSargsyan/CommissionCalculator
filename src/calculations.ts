...
import { type ITransaction, type IConfig } from './index.interface'
...
export const cashOutNaturalCommission = (
  ...
  const transactionIndex = userTransactionsByWeek.indexOf(transaction);
  ...
  for (let i = 0; i < userTransactionsByWeek.length; i++) {
    const item = userTransactionsByWeek[i]
